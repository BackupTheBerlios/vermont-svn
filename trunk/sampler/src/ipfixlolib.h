 ...
 *
 * Header for the ipfixlolib
 *
 * WARNING: THIS HEADER IS AN API ONLY. IT IS SUBJECT TO CHANGE!!!
 *
...
#ifndef IPFIX_LOLIB_H
#define IPFIX_LOLIB_H
...
typedef struct {
  ...
  char* set_header_store;  /* buffer to store the header */
  int set_header_capacity;  /* total capacity of the header */
  int set_header_length; /* used length of the header store */
  int data_length;
 
  // uint16_t* set_id; // do we need this?
} ipfix_set_manager;
...
// gerhard: exporter als referenz bergeben, um ihn auf NULL setzen zu k�nen!
