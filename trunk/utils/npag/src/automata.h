...
enum e_states {BEGIN = 1, STREAM, TRAFFIC, PATTERN, IP, IP6, TCP, UDP, ICMP, ICMP6};
...
struct s_node {
	...
	void (*init)(struct s_config *conf);
	void (*check)(struct s_config *conf);
	void (*read)(char *word, char *val, struct s_config *conf, struct s_automata *automata);
	struct s_node *next;	
}head;
