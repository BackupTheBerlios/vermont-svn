...
void Rijndael::keySched(UINT8 key[_MAX_KEY_COLUMNS][4])
{
	...
	for(j = 0;(j < uKeyColumns) && (r <= static_cast<int>(m_uRounds)); )
	{
	...
	while(r <= static_cast<int>(m_uRounds))
	{
...
void Rijndael::keyEncToDec()
{
	...
	for(r = 1; r < static_cast<int>(m_uRounds); r++)
	{
		w = m_expandedKey[r][0];
...
void Rijndael::encrypt(const UINT8 a[16], UINT8 b[16])
{
    ...
    *((UINT32*)(b +12)) = *((UINT32*)T1[temp[3][0]])
						...
						^ *((UINT32*)T3[temp[1][2]]) 
						^ *((UINT32*)T4[temp[2][3]]);
	for(r = 1; r < static_cast<int>(m_uRounds)-1; r++)
	{
