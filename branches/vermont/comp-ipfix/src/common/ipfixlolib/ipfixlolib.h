...
/* Struct containing an compressed ipfix-header 
 *     Header Format (see draft-... TODO: reference documents)
 * TODO: document final format
 */
...
typedef struct {
    uint8_t set_id;
    uint8_t length;
} ipfix_compressed_set_header;

/*
 * Indicates the compression level used to compress the IPFIX header for 
 * compressed IPFIX.
 * C_EMPTY = field is not accessible
 * C_SINGLE = field is compressed to a single byte
 * C_MIDDLE = field is compressed to two bytes
 * C_ORIG   = field is not compressed
 */
enum compression_state { C_EMPTY, C_SINGLE, C_MIDDLE, C_ORIG };

#endif //SUPPORT_COMPRESSED_IPFIX
...
#ifdef SUPPORT_COMPRESSED_IPFIX
	/* Buffer to store the compressed headers.
           The compressed header is a compressed copy of the ipfix_set_header. 
           It is only used to fill the sendbuffer io_vecs
	*/
	ipfix_compressed_set_header compressed_set_header_store[IPFIX_MAX_SETS_PER_PACKET];
#endif
...
typedef struct {
	...
	ipfix_header packet_header; /* header struct for standard IPFIX */
#ifdef SUPPORT_COMPRESSED_IPFIX
	ipfix_compressed_header compressed_packet_header; /* header struct for compressed IPFIX */
#endif //SUPPORT_COMPRESSED_IPFIX
...
/*
 * Each exporting process is associated with a sequence number and a observation domain ID
...
#ifdef SUPPORT_COMPRESSED_IPFIX
	int  compressed_ipfix; // send compressed IPFIX messages instead of IPFIX
	enum compression_state compress_sequence_no; // level of sequence number compression for compressed ipfix
	enum compression_state compress_export_time; // level of export time compression for compressed ipfix
#endif
