...
do
  ...
  do
    CUSUMFILE=$(echo $METRICSFILE | sed "s/\_metrics\.txt/\.$METRIC\_metric.cusumparams.txt/")
    ...
    echo set xrange [0:] >> graphs.gnuplot
    echo set logscale y2 >> graphs.gnuplot
    echo plot \'$METRICSFILE\' using $TESTRUNCOLUMN:$COLUMN title \'Value\' with lines, \
      \'$METRICSFILE\' using $TESTRUNCOLUMN:$COLUMN title \'Value \(log\)\' axes x1y2 with points  >> graphs.gnuplot
    ...
    echo set xrange [0:] >> graphs.gnuplot
    echo plot \'$CUSUMFILE\' using 7:2 title \'g\' with lines, \
      \'$CUSUMFILE\' using 7:6 title \'alarms\' axes x1y2 with points, \
      \'$CUSUMFILE\' using 7:3 title \'N\' with lines >> graphs.gnuplot
    # WMW from WKPFILE
...
for CUSUMFILE in *_metric.cusumparams.txt
do
  METRIK=$(echo $CUSUMFILE | sed 's/\.cusumparams.txt//')
  WKPFILE=$(echo $CUSUMFILE | sed 's/cusum/wkp/')
  # plot all graphs
  echo set size 1, 1 >> graphs.gnuplot
  echo set origin 0, 0 >> graphs.gnuplot
  echo set title \'$METRIK\' >> graphs.gnuplot
  echo set multiplot >> graphs.gnuplot
  echo set rmargin 10 >> graphs.gnuplot
  echo set lmargin 10 >> graphs.gnuplot
  # Value
  ...
  echo set size 1, 0.4 >> graphs.gnuplot
  echo set xrange [0:] >> graphs.gnuplot
  echo plot \'$CUSUMFILE\' using 7:1 title \'Value\' with lines >> graphs.gnuplot
  # CUSUM
  #echo set title \'CUSUM parameters\' >> graphs.gnuplot
  echo unset title >> graphs.gnuplot
  echo unset xtics >> graphs.gnuplot
  ...
  echo set xrange [0:] >> graphs.gnuplot
  echo plot \'$CUSUMFILE\' using 7:2 title \'g\' with lines, \
    \'$CUSUMFILE\' using 7:6 with points title \'alarms\' axes x1y2, \
    \'$CUSUMFILE\' using 7:3 title \'N\' with lines >> graphs.gnuplot
  # WMW
  #echo set title \'WMW parameters\' >> graphs.gnuplot
  echo set origin 0, 0.32 >> graphs.gnuplot
