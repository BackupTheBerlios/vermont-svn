...
void copyUintNetByteOrder(FieldData* dest, char* src, ExpressFieldType type);
...
int dbReaderSendNewTemplate(IpfixDbReader* ipfixDbReader,ExpressDataTemplateInfo* dataTemplateInfo, int table_index);
int dbReaderSendTable(IpfixDbReader* ipfixDbReader, ExpressDataTemplateInfo* dataTemplateInfo, int n);
int dbReaderDestroyTemplate(IpfixDbReader* ipfixDbReader, ExpressDataTemplateInfo* dataTemplateInfo);
...
void* readFromDB(void* ipfixDbReader_)
{
	...
	ExpressDataTemplateInfo* dataTemplateInfo = (ExpressDataTemplateInfo*)malloc(sizeof(ExpressDataTemplateInfo));
...
int dbReaderSendNewTemplate(IpfixDbReader* ipfixDbReader,ExpressDataTemplateInfo* dataTemplateInfo, int table_index)
{
	...
	for(i = 0; i < dbData->colCount; i++) {			
		...
		dataTemplateInfo->fieldInfo = realloc(dataTemplateInfo->fieldInfo,
						      sizeof(ExpressFieldInfo)*dataTemplateInfo->fieldCount);
		ExpressFieldInfo* fi = &dataTemplateInfo->fieldInfo[dataTemplateInfo->fieldCount - 1];	
	...
	for (n = 0; n != dbReader->callbackCount; n++) {
		ExpressCallbackInfo* ci = &dbReader->callbackInfo[n];
...
int dbReaderSendTable(IpfixDbReader* ipfixDbReader, ExpressDataTemplateInfo* dataTemplateInfo, int table_index)
{
	...
	while((dbRow = mysql_fetch_row(dbResult))) {
		...
		for (i = 0; i != dbReader->callbackCount; ++i) {
			ExpressCallbackInfo* ci = &dbReader->callbackInfo[i];
 ...
 */
void addIpfixDbReaderCallbacks(IpfixDbReader* ipfixDbReader, ExpressCallbackInfo handles) 
{
	DbReader* dbReader = ipfixDbReader->dbReader;
	int n = ++dbReader->callbackCount;
	dbReader->callbackInfo = (ExpressCallbackInfo*)realloc(dbReader->callbackInfo, 
							n * sizeof(ExpressCallbackInfo));
	memcpy(&dbReader->callbackInfo[n-1], &handles, sizeof(ExpressCallbackInfo));
}
