...
int writeDataRecord(void* ipfixDbWriter,Exp_SourceID* sourceID, ExpressTemplateInfo* templateInfo, uint16_t length, FieldData* data);
int writeDataDataRecord(void* ipfixDbWriter,Exp_SourceID* sourceID, ExpressDataTemplateInfo* dataTemplateInfo, uint16_t length, FieldData* data);
char* getRecData(IpfixDbWriter* ipfixDbWriter,Table* table,Exp_SourceID* sourceID,ExpressDataTemplateInfo* dataTemplateInfo,uint16_t length,FieldData* data);

int getExporterID(IpfixDbWriter* ipfixDbWriter,Table* table, Exp_SourceID* sourceID);
...
uint64_t getdata(ExpressFieldType type, FieldData* data);
uint64_t getIPFIXValue(ExpressFieldType type, FieldData* data);
uint32_t getdefaultIPFIXdata(int ipfixtype);

uint32_t getipv4address(ExpressFieldType type, FieldData* data);
...
*/
int  writeDataDataRecord(void* ipfixDbWriter_, Exp_SourceID* sourceID, ExpressDataTemplateInfo* dataTemplateInfo, uint16_t length, FieldData* data)
{
...
/**
*	loop over the ExpressDataTemplateInfo (fieldinfo,datainfo) to get the IPFIX values to store in database
*/
char* getRecData(IpfixDbWriter* ipfixDbWriter,Table* table, Exp_SourceID* sourceID,
		 ExpressDataTemplateInfo* dataTemplateInfo,uint16_t length, FieldData* data)
{
	int i ,j, k, n;
	uint64_t intdata = 0;
...
ExpressCallbackInfo getIpfixDbWriterCallbackInfo(IpfixDbWriter *ipfixDbWriter) {
