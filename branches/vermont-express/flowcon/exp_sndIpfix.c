...
int sndNewDataTemplate(void* ipfixSender_, Exp_SourceID* sourceID, ExpressDataTemplateInfo* dataTemplateInfo) {
	...
	*p = htons(my_template_id);
	...
	for (i = 0; i < dataTemplateInfo->fieldCount; i++) {
		ExpressFieldInfo* fi = &dataTemplateInfo->fieldInfo[i];
	...
	for (i = 0; i < dataTemplateInfo->dataCount; i++) {
		ExpressFieldInfo* fi = &dataTemplateInfo->dataInfo[i];
		if ((fi->type.id == IPFIX_TYPEID_sourceIPv4Address) && (fi->type.length == 5)) {
			splitFixedfields++;
		}
		else if ((fi->type.id == IPFIX_TYPEID_destinationIPv4Address) && (fi->type.length == 5)) {
...
ExpressCallbackInfo ExpressgetIpfixSenderCallbackInfo(ExpressIpfixSender* ipfixSender) {
	...
	ci.dataTemplateCallbackFunction = ExpresssndNewDataTemplate;
	ci.dataDataRecordCallbackFunction = ExpresssndDataDataRecord;
	ci.dataTemplateDestructionCallbackFunction = ExpresssndDestroyDataTemplate;
	return ci;
}
