...
void Express_sampler_hook_entry(void *ctx, void *data)
{
	...
	if(ph->transport_header == NULL) {
	    ExpressaggregateDataRecord(aggregator, HOOK_SOURCE_ID, &ip_traffic_template, ph->length, fdata);
	}
	else
	{
		...
		case IPFIX_protocolIdentifier_ICMP:
		    ...
		    ExpressaggregateDataRecord(aggregator, HOOK_SOURCE_ID, &icmp_traffic_template, ph->length, fdata);
		...
		case IPFIX_protocolIdentifier_UDP:
		    ...
		    ExpressaggregateDataRecord(aggregator, HOOK_SOURCE_ID, &udp_traffic_template, ph->length, fdata);
		...
		case IPFIX_protocolIdentifier_TCP:
		    ...
		    ExpressaggregateDataRecord(aggregator, HOOK_SOURCE_ID, &tcp_traffic_template, ph->length, fdata);
		    /* reset offsets for srcport/dstport to starting values */
