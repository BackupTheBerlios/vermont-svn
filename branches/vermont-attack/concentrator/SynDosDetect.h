	...
	public:
		int checkForAttack(const Packet* p)
		{
			...
			if (now != lastCheck) 
			{
				...
				//	fprintf(stderr,"expected in %.2f out %.2f\n",exp_ratio_in,exp_ratio_out);
				//	fprintf(stderr,"ratio in %.2f out %.2f\n",((double) Incoming.varCountIn + 1.0) / ((double) Outgoing.varCountOut + 1.0),((double) Outgoing.varCountIn + 1.0) / ((double) DdosIncoming.varCountOut + 1.0));
				//	fprintf(stderr,"threshold in %.2f out %.2f\n",((double) Incoming.expectedValueIn/5.0 + 500) / ((double) Outgoing.expectedValueOut/5.0 + 1.0),((double) Outgoing.expectedValueIn /5.0 + 500) / ((double) DdosIncoming.expectedValueOut /5.0 + 1.0));

				//		std::cerr << Outgoing.varCountIn << " " << Incoming.varCountOut << " " << DdosIncoming.varCountIn << " " << Outgoing.varCountOut << std::endl;
				//fprintf(stderr,"Incoming ratio: %.2f\n",((double) Incoming.varCountIn + (double) Outgoing.varCountIn + 1.0) / ((double) DdosIncoming.varCountOut + (double) Outgoing.varCountOut + 1.0));
