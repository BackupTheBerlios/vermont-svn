 ...
 */
int sndNewTemplate(void* ipfixSender_, SourceID sourceID, TemplateInfo* templateInfo) {
	...
	*p = htons(my_template_id);
	...
	if (0 != ipfix_start_template_set(exporter, my_template_id, templateInfo->fieldCount + splitFields)) {
		fatal("ipfix_start_template_set failed");
		goto fail3;
		}
 ...
 */
int sndDestroyTemplate(void* ipfixSender_, SourceID sourceID, TemplateInfo* templateInfo) {
 ...
 */
int sndDataRecord(void* ipfixSender_, SourceID sourceID, TemplateInfo* templateInfo, uint16_t length, FieldData* data) {
	IpfixSender* ipfixSender = ipfixSender_;
	...
	uint16_t my_n_template_id = *(uint16_t*)templateInfo->userData;
...
/**
 * Put new Data Data Record in outbound exporter queue
 * @param ipfixSender_ handle to the Exporter
 * @param sourceID ignored
 * @param dataTemplateInfo Pointer to a structure defining the DataTemplate used
 * @param length Length of the data block supplied
